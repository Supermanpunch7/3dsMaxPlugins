...
static const Class_ID wrapperID(0x71293b59, 0x19374a79);
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferProSeamIsolated"); }
    const TCHAR* NonLocalizedClassName() override { return ClassName(); }
    const TCHAR* InternalName() override { return _T("ChamferProSeamIsolated"); }
...
public:
    void BeginEditParams(IObjParam* ip, ULONG flags, Animatable* previous) override {
        ...
        selector = ip->AddRollupPage(instance, MAKEINTRESOURCE(201), PanelProc,
            _T("ChamferProSeamIsolated"), reinterpret_cast<LPARAM>(this));
    ...
    Class_ID ClassID() override { return wrapperID; }
    SClass_ID SuperClassID() override { return OSM_CLASS_ID; }
    void GetClassName(MSTR& s, bool) const override { s = _T("ChamferProSeamIsolated"); }
    const TCHAR* GetObjectName(bool) const override { return _T("ChamferProSeamIsolated"); }
    ...
    void ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node) override {
        ...
        auto* data = static_cast<PairContext*>(mc.localData);
        // Keep the wrapper input as the final boundary reference. Stage two's
        // local correspondence alone can lose the original longitudinal owner.
        MNMesh originalMesh;
        const bool haveOriginal = os->obj->IsSubClassOf(polyObjectClassID) != 0;
        if (haveOriginal) originalMesh = static_cast<PolyObject*>(os->obj)->GetMesh();
        bool ranSecond = false;
        for (int i = 0; i < 2; ++i) {
            auto* m = Stage(i);
            if (!m || !m->IsEnabled()) continue;
            if (i == 1) ranSecond = true;
...
if (haveSource && os->obj->IsSubClassOf(polyObjectClassID))
    LogNativeHardTrial(i, ApplyNativeHardTrial(sourceMesh, static_cast<PolyObject*>(os->obj)->GetMesh()));
        ...
        if (ranSecond && haveOriginal && os->obj->IsSubClassOf(polyObjectClassID))
            LogNativeHardTrial(2, ApplyNativeHardTrial(originalMesh, static_cast<PolyObject*>(os->obj)->GetMesh()));
    }
};
