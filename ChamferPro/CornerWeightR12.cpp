...
static const Class_ID wrapperID(0x71293b6e, 0x19374a95);
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferProCornerWeightR12"); }
    const TCHAR* NonLocalizedClassName() override { return ClassName(); }
    const TCHAR* InternalName() override { return _T("ChamferProCornerWeightR12"); }
...
class NativePair : public Modifier {
    ...
    static INT_PTR CALLBACK PanelProc(HWND window, UINT message,
        ...
        if (message == WM_NCDESTROY && self) {
            // Max may tear down command-panel pages without a matching
            // EndEditParams call.  Drop the stale bridge handle so the
            // selector is recreated the next time this modifier is edited.
    ...
    static NativePair* uiOwner;
    bool closingEditor = false;

    void CloseEditUI(Animatable* next) {
        ...
        if (owner) {
            if (auto* modifier = Stage(activeStage))
                modifier->EndEditParams(owner, END_EDIT_REMOVEUI, next);
            // AddRollupPage returns a Max/Qt bridge handle for which Win32
            // IsWindow() is not a reliable ownership test in Max 2027.
            if (page) owner->DeleteRollupPage(page);
        }
        if (uiOwner == this) uiOwner = nullptr;
        closingEditor = false;
    }
...
public:
    void BeginEditParams(IObjParam* ip, ULONG flags,
        ...
        // Only one wrapper instance may own command-panel pages.  This also
        // closes pages left by switching between copied modifier instances.
        if (uiOwner == this && editIP == ip && selector)
            return;
        if (uiOwner && uiOwner != this)
            uiOwner->CloseEditUI(nullptr);
        if (editIP || selector)
            CloseEditUI(nullptr);
        ...
        selector = ip->AddRollupPage(
            ...
            _T("ChamferProCornerWeightR12"), reinterpret_cast<LPARAM>(this));
        uiOwner = this;
        if (auto* modifier = Stage(activeStage))
            modifier->BeginEditParams(ip, flags, previous);
    }

    void EndEditParams(IObjParam*, ULONG, Animatable* next) override {
        CloseEditUI(next);
    }
    ...
    ~NativePair() override {
        if (editIP || selector || uiOwner == this)
            CloseEditUI(nullptr);
        DeleteAllRefsFromMe();
    }
    ...
    void GetClassName(MSTR& name, bool) const override {
        name = _T("ChamferProCornerWeightR12");
    }
    const TCHAR* GetObjectName(bool) const override {
        return _T("ChamferProCornerWeightR12");
    }
    ...
    void ModifyObject(TimeValue time, ModContext& context,
        ...
        // When both native stages run, keep the original face-id map alive
        // across stage one.  The final pass can then resolve stage-two children
        // directly against the first parent instead of inheriting an
        // intermediate edge that still has the native default Weight.
        auto* firstStage = Stage(0);
        auto* secondStage = Stage(1);
        bool preserveOriginalLineage =
            haveOriginal && firstStage && firstStage->IsEnabled() &&
            secondStage && secondStage->IsEnabled();
        WeightFaceLineageTag originalWeightTag;
        if (preserveOriginalLineage) {
            auto& inputMesh =
                static_cast<PolyObject*>(state->obj)->GetMesh();
            originalWeightTag = BeginWeightFaceLineage(inputMesh);
            preserveOriginalLineage = originalWeightTag.channel >= 0;
        }
        bool originalTagActive = preserveOriginalLineage;
        ...
        for (int stage = 0; stage < 2; ++stage) {
            ...
            if (haveSource) {
                ...
                weightTag = preserveOriginalLineage && stage == 0
                    ? originalWeightTag
                    : BeginWeightFaceLineage(inputMesh);
            }
            ...
            const bool retainTag =
                preserveOriginalLineage && stage == 0;
            LogNativeCornerTrial(
                10 + stage,
                ApplyWeightFaceLineage(
                    sourceMesh, outputMesh, weightTag, !retainTag));

            if (preserveOriginalLineage && stage == 1) {
                LogNativeCornerTrial(
                    20,
                    ApplyWeightFaceLineage(
                        originalMesh, outputMesh, originalWeightTag));
                originalTagActive = false;
            }
        }

        if (originalTagActive &&
            state->obj->IsSubClassOf(polyObjectClassID)) {
            EndWeightFaceLineage(
                static_cast<PolyObject*>(state->obj)->GetMesh(),
                originalWeightTag);
        }
    }
...
NativePair* NativePair::uiOwner = nullptr;
