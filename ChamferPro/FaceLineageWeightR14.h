...
inline UVVert WeightFaceSignature(int face) {
    unsigned int h = static_cast<unsigned int>(face + 1) * 2654435761u;
    h ^= h >> 16;
    return UVVert(float(face + 1), float(h & 65535u), float((h >> 16) & 65535u));
}
...
inline WeightFaceLineageTag BeginWeightFaceLineage(MNMesh& mesh) {
    ...
    for (int face = 0; face < mesh.numf; ++face) {
        tags->v[face] = WeightFaceSignature(face);
...
inline int DecodeWeightFaceTag(const UVVert& value, int sourceFaceCount) {
    ...
    if (encoded < 1 || encoded > sourceFaceCount ||
        ...
        return -1;
    const UVVert signature = WeightFaceSignature(encoded - 1);
    if (std::fabs(value.y - signature.y) > 0.01f ||
        std::fabs(value.z - signature.z) > 0.01f) return -1;
    return encoded - 1;
}
...
inline int ApplyWeightFaceLineage(const MNMesh& source, MNMesh& output,
    ...
    // Propagate exact parents along opposite edges of generated quad strips.
    // Never cross surviving source faces or resolve a conflict by edge order.
    // Do this before the lower-confidence junction fallback.
    auto propagateStripParents = [&]() {
    for (int pass = 0; pass < output.nume; ++pass) {
        std::vector<int> proposed(output.nume, -1);
        bool changed = false;
        for (int fi = 0; fi < output.numf; ++fi) {
            const MNFace& f = output.f[fi];
            if (f.GetFlag(MN_DEAD) || f.deg != 4 || faceOwner[fi] >= 0) continue;
            for (int k = 0; k < 4; ++k) {
                const int a = f.edg[k], b = f.edg[(k + 2) % 4];
                if (a < 0 || b < 0 || a >= output.nume || b >= output.nume ||
                    parent[a] < 0 || parent[b] >= 0) continue;
                const MNEdge& e = output.e[b];
                const MNEdge& se = source.e[parent[a]];
                const Point3 d = output.v[e.v2].p - output.v[e.v1].p;
                const Point3 sd = source.v[se.v2].p - source.v[se.v1].p;
                const float lengths = Length(d) * Length(sd);
                if (lengths <= 1.e-12f || std::fabs(DotProd(d, sd)) < 0.95f * lengths) continue;
                if (proposed[b] == -1) proposed[b] = parent[a];
                else if (proposed[b] != parent[a]) proposed[b] = -2;
            }
        }
        for (int e = 0; e < output.nume; ++e) {
            if (parent[e] < 0 && proposed[e] >= 0) {
                parent[e] = proposed[e];
                changed = true;
            }
        }
    ...
    for (int edge = 0; edge < output.nume; ++edge) {
        ...
        // Parent identity must be independent of the channel's scalar value.
        // Equal Weight does not establish equal Crease/Depth or ancestry.
        if (aligned.size() == 1)
            parent[edge] = aligned.front().edge;
    }

    // Junction resolution creates new seeds. Carry those seeds through all
    // parallel segment rows too, rather than leaving the interior at defaults.
    propagateStripParents();
