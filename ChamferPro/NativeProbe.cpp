...
static const Class_ID wrapperID(0x71293b44, 0x19374a65);
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferProNativeProbe"); }
    const TCHAR* NonLocalizedClassName() override { return ClassName(); }
    const TCHAR* InternalName() override { return _T("ChamferProNativeProbe"); }
...
public:
    void BeginEditParams(IObjParam* ip, ULONG flags, Animatable* previous) override {
        ...
        selector = ip->AddRollupPage(instance, MAKEINTRESOURCE(201), PanelProc,
            _T("ChamferProNativeProbe"), reinterpret_cast<LPARAM>(this));
    ...
    Class_ID ClassID() override { return wrapperID; }
    SClass_ID SuperClassID() override { return OSM_CLASS_ID; }
    void GetClassName(MSTR& s, bool) const override { s = _T("ChamferProNativeProbe"); }
    const TCHAR* GetObjectName(bool) const override { return _T("ChamferProNativeProbe"); }
    ...
    void ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node) override {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            NativeProbe(os, i, true); m->ModifyObject(t, data->stages[i], os, node); NativeProbe(os, i, false);
        }
    }
};
