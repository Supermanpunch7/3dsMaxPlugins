...
static const Class_ID CHAMFERPRO_CLASS_ID(0x71293b42, 0x19374a63);
...
public:
    ...
    {
        s = _T("ChamferproLineageLab");
    }

    Class_ID ClassID() override { return CHAMFERPRO_CLASS_ID; }
    SClass_ID SuperClassID() override { return OSM_CLASS_ID; }
    const TCHAR* GetObjectName(bool localized) const override { return _T("ChamferproLineageLab"); }
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferproLineageLab"); }
    const TCHAR* NonLocalizedClassName() override { return _T("ChamferproLineageLab"); }
    SClass_ID SuperClassID() override { return OSM_CLASS_ID; }
    Class_ID ClassID() override { return CHAMFERPRO_CLASS_ID; }
    const TCHAR* Category() override { return _T("Supermanpunch"); }
    const TCHAR* InternalName() override { return _T("ChamferproLineageLab"); }
...
bool ChamferProMod::ApplyOneChamferPass(MNMesh& mesh, DWORD flag, const ChamferPassSettings& settings, bool limitEffect)
{
    ...
    // FillInMesh may recreate MNEdges: track and array order are not lineage.
    // Capture identity using compacted endpoint IDs before rebuilding.
    std::map<std::pair<int,int>, int> compactOwners;
    for (int e = 0; e < mesh.nume; ++e) {
        int a = mesh.e[e].v1, b = mesh.e[e].v2;
        if (a > b) std::swap(a,b);
        if (!compactOwners.emplace(std::make_pair(a,b), compactE[e]).second)
            return false; // Parallel edges require a face-aware identity.
    }
    mesh.FillInMesh();
    if (int(compactV.size()) != mesh.numv) return false;
    for (int v = 0; v < mesh.numv; ++v) mesh.v[v].orig = compactV[v];
    mesh.ClearEFlags(MN_EDGE_WAS_PROCESSED);
    for (int e = 0; e < mesh.nume; ++e) {
        int a = mesh.e[e].v1, b = mesh.e[e].v2;
        if (a > b) std::swap(a,b);
        auto owner = compactOwners.find(std::make_pair(a,b));
        if (owner == compactOwners.end()) return false;
        mesh.e[e].track = owner->second;
        if (owner->second >= 0 && source.e[owner->second].GetFlag(flag))
            mesh.e[e].SetFlag(MN_EDGE_WAS_PROCESSED);
    }
    transferData();
    ...
    if (!LineageBuildNormals(mesh, outputHard)) return false;
    // Exact per-parent checks, not range counts (Depth defaults can match a range).
    for (int e = 0; e < mesh.nume; ++e) {
        const int parent = mesh.e[e].track;
        if (parent < 0) continue; // Explicitly unresolved, never claimed as inherited.
        for (int ch = 0; ch < source.EDNum(); ++ch)
            if (source.eDataSupport(ch) &&
                (!mesh.eDataSupport(ch) || mesh.edgeFloat(ch)[e] != source.edgeFloat(ch)[parent]))
                return false;
        if (LineageHard(mesh, e) != sourceHard[parent]) return false;
    }
    wchar_t path[1024];
