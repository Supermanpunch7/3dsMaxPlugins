...
static const Class_ID wrapperID(0x71293b6d, 0x19374a94);
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferProCornerWeightR11"); }
    const TCHAR* NonLocalizedClassName() override { return ClassName(); }
    const TCHAR* InternalName() override { return _T("ChamferProCornerWeightR11"); }
    ...
    static INT_PTR CALLBACK PanelProc(HWND window, UINT message,
        ...
        if (message == WM_APP + 1 && self && self->editIP &&
            ...
            if (auto* old = self->Stage(self->activeStage))
                old->EndEditParams(self->editIP, self->editFlags, nullptr);
            self->activeStage = int(w);
...
public:
    void BeginEditParams(IObjParam* ip, ULONG flags,
        ...
        // Max can re-enter BeginEditParams while this wrapper is already the
        // active stack item (stack refresh, copied nodes, reference messages).
        // Reusing the live page prevents identical selector rollouts from
        // accumulating above the native Chamfer parameters.
        if (editIP == ip && selector && IsWindow(selector))
            return;

        // Recover from an unmatched/stale edit session before opening a new
        // one.  Keep the native child editor paired with our own rollup.
        if (editIP) {
            if (auto* modifier = Stage(activeStage))
                modifier->EndEditParams(editIP, editFlags, nullptr);
            if (selector && IsWindow(selector))
                editIP->DeleteRollupPage(selector);
            selector = nullptr;
            editIP = nullptr;
        }
        ...
        selector = ip->AddRollupPage(
            ...
            _T("ChamferProCornerWeightR11"), reinterpret_cast<LPARAM>(this));
        if (auto* modifier = Stage(activeStage))
            modifier->BeginEditParams(ip, flags, previous);
    }

    void EndEditParams(IObjParam* ip, ULONG flags, Animatable* next) override {
        IObjParam* owner = editIP ? editIP : ip;
        if (editIP) {
            if (auto* modifier = Stage(activeStage))
                modifier->EndEditParams(owner, flags, next);
        }
        if (selector && IsWindow(selector))
            owner->DeleteRollupPage(selector);
        selector = nullptr;
        editIP = nullptr;
    }
    ...
    void GetClassName(MSTR& name, bool) const override {
        name = _T("ChamferProCornerWeightR11");
    }
    const TCHAR* GetObjectName(bool) const override {
        return _T("ChamferProCornerWeightR11");
    }
