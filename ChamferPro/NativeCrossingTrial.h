...
inline int ApplyNativeCrossingTrial(MNMesh& src, MNMesh& dst) {
...
inline void LogNativeCrossingTrial(int stage, int status) {
    wchar_t path[2048];
    DWORD n=GetEnvironmentVariableW(L"CHAMFERPRO_CROSSING_LOG",path,2048);
    if (!n || n>=2048) return;
    FILE* f=nullptr;
