...
static const Class_ID wrapperID(0x71293b48, 0x19374a69);
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferProCrossingTrial"); }
    const TCHAR* NonLocalizedClassName() override { return ClassName(); }
    const TCHAR* InternalName() override { return _T("ChamferProCrossingTrial"); }
...
public:
    void BeginEditParams(IObjParam* ip, ULONG flags, Animatable* previous) override {
        ...
        selector = ip->AddRollupPage(instance, MAKEINTRESOURCE(201), PanelProc,
            _T("ChamferProCrossingTrial"), reinterpret_cast<LPARAM>(this));
    ...
    Class_ID ClassID() override { return wrapperID; }
    SClass_ID SuperClassID() override { return OSM_CLASS_ID; }
    void GetClassName(MSTR& s, bool) const override { s = _T("ChamferProCrossingTrial"); }
    const TCHAR* GetObjectName(bool) const override { return _T("ChamferProCrossingTrial"); }
...
if (haveSource && os->obj->IsSubClassOf(polyObjectClassID))
    LogNativeCrossingTrial(i, ApplyNativeCrossingTrial(sourceMesh, static_cast<PolyObject*>(os->obj)->GetMesh()));
        }
    }
};
