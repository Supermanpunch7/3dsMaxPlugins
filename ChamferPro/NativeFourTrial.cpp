...
static const Class_ID wrapperID(0x71293b46, 0x19374a67);
...
public:
    ...
    const TCHAR* ClassName() override { return _T("ChamferProFourTrial"); }
    const TCHAR* NonLocalizedClassName() override { return ClassName(); }
    const TCHAR* InternalName() override { return _T("ChamferProFourTrial"); }
...
public:
    void BeginEditParams(IObjParam* ip, ULONG flags, Animatable* previous) override {
        ...
        selector = ip->AddRollupPage(instance, MAKEINTRESOURCE(201), PanelProc,
            _T("ChamferProFourTrial"), reinterpret_cast<LPARAM>(this));
    ...
    Class_ID ClassID() override { return wrapperID; }
    SClass_ID SuperClassID() override { return OSM_CLASS_ID; }
    void GetClassName(MSTR& s, bool) const override { s = _T("ChamferProFourTrial"); }
    const TCHAR* GetObjectName(bool) const override { return _T("ChamferProFourTrial"); }
...
if (haveSource && os->obj->IsSubClassOf(polyObjectClassID))
    LogNativeFourTrial(i, ApplyNativeFourTrial(sourceMesh, static_cast<PolyObject*>(os->obj)->GetMesh()));
        }
    }
};
