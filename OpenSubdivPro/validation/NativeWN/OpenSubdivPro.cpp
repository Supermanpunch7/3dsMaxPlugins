...
constexpr Class_ID kOpenSubdivProClassId(0x38a75c2e, 0x71e946b4);
...
public:
    ...
    const TCHAR* NonLocalizedClassName() override { return _T("OpenSubdivProWNValidation"); }
    ...
    const TCHAR* InternalName() override { return _T("OpenSubdivProWNValidation"); }
...
public:
    ...
    Modifier* EnsureNativeNormals(TimeValue t)
    {
        if (auto* native = NativeNormals())
            return native;
        auto* native = static_cast<Modifier*>(CreateInstance(OSM_CLASS_ID, WEIGHTED_NORMALS_MOD_ID));
        if (!native)
            return nullptr;
    ...
    {
        return localized ? GetString(IDS_CLASS_NAME) : _T("OpenSubdivProWNValidation");
    }
    ...
    void BeginEditParams(IObjParam* ip, ULONG flags, Animatable* prev) override
    {
        ...
        if (auto* native = EnsureNativeNormals(ip->GetTime()))
            native->BeginEditParams(ip, flags, nullptr);
    ...
    void EndEditParams(IObjParam* ip, ULONG flags, Animatable* next) override
    {
        ...
        if (auto* native = NativeNormals())
            native->EndEditParams(ip, END_EDIT_REMOVEUI, nullptr);
    ...
    int Display(TimeValue t, INode* node, ViewExp* view, int flags, ModContext* context) override
    {
        ...
        if (!native || !data || !data->evaluated || !m_pblock->GetInt(pb_smooth_result, t))
            return 0;
        return native->Display(t, node, view, flags, &data->normals);
    }
    ...
    void GetWorldBoundBox(TimeValue t, INode* node, ViewExp* view, Box3& box, ModContext* context) override
    {
        ...
        auto* data = context ? static_cast<NativeNormalsContext*>(context->localData) : nullptr;
        if (native && data && data->evaluated && m_pblock->GetInt(pb_smooth_result, t))
            native->GetWorldBoundBox(t, node, view, box, &data->normals);
    }
...
public:
    WeightedNormalsRollout()
    {
        ...
        auto* enable = new QCheckBox(tr("Enable Weighted Normals"), this);
        enable->setObjectName(QStringLiteral("smoothResult"));
        root->addWidget(enable);
        auto* hint = new QLabel(tr("Settings and Display Normals use the native Weighted Normals rollout."), this);
    ...
    int& rollupCategory)
{
    ...
    case map_weighted_normals:
        rollupTitle = _T("Weighted Normals - Enable / Hard Edges");
